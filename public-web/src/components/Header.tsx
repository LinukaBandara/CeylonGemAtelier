      ...
      <div className="mx-auto max-w-[1440px] px-6 md:px-12 lg:px-20 h-16 md:h-20 flex items-center justify-between">
        ...
        >
          <div className="w-6 h-5 relative flex flex-col justify-between">
            <span className={`block h-[1.5px] w-full bg-[var(--color-graphite)] transition-all duration-300 origin-center ${menuOpen ? "rotate-45 translate-y-[9px]" : ""}`} />
            <span className={`block h-[1.5px] w-full bg-[var(--color-graphite)] transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`} />
            <span className={`block h-[1.5px] w-full bg-[var(--color-graphite)] transition-all duration-300 origin-center ${menuOpen ? "-rotate-45 -translate-y-[9px]" : ""}`} />
          </div>
        </button>
      </div>

      <div className={`md:hidden fixed inset-0 top-16 bg-[var(--color-ivory)] transition-opacity duration-500 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <nav className="flex flex-col items-center justify-center h-full gap-12 px-8 pb-32">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={closeMenu}
              className={`font-serif text-2xl transition-colors duration-300 ${isActive(link.href) ? "text-[var(--color-graphite)]" : "text-[var(--color-muted)]"}`}
            >
              {link.label}
            </Link>
          ))}
